// Copyright (C) 2019-2026 Artur Kurpukov
...
class update : Form {
    ...
    private UInt16 CRC16(UInt16 oldCRC, byte data) {
        uint m = (((uint)oldCRC << 8) | data);
        for (int n = 0; n < 8; n++) {
            m <<= 1;
            if ((m & 0x1000000) != 0) {
                m ^= 0x800500;
    ...
    private UInt32 CRC32(UInt32 oldCRC, byte data) {
        oldCRC ^= data;
        for (int n = 0; n < 8; n++) {
            if ((oldCRC & 0x00000001) != 0) {
                oldCRC = ((oldCRC >> 1) ^ 0xEDB88320);
            } else {
                oldCRC >>= 1;
            }
        }
    ...
    private void BackgroundWorker1DoWork(object sender, DoWorkEventArgs e) {
        ...
        try {
            ...
            for (int index = 0; index < size;) {
                ...
                UInt16 crc = 0x0000;
                do {
                    crc = CRC16(crc, buf[index++]);
                } while (++frameSize < 16 || index < 48);
                
                crc = CRC16(crc, 0);
                crc = CRC16(crc, 0);
                
                _serialPort1.Write(new byte[1] { (byte)(frameSize+2) }, 0, 1);
                _serialPort1.Write(buf, (index-frameSize), frameSize);
                _serialPort1.Write(new byte[2] { (byte)(crc >> 8), (byte)crc }, 0, 2);
